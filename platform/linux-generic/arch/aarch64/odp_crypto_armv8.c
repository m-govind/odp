...
struct odp_crypto_generic_session_t {
	...
	struct {
		...
		uint8_t iv_data[_ARM_CRYPTO_MAX_IV_LENGTH];
		uint8_t key_data[_ARM_CRYPTO_MAX_CIPHER_KEY_LENGTH];
		crypto_func_t func;
	} cipher;

	struct {
		uint8_t  key[_ARM_CRYPTO_MAX_AUTH_KEY_LENGTH];
		uint8_t  iv_data[_ARM_CRYPTO_MAX_IV_LENGTH];
		crypto_func_t func;
	} auth;
	unsigned int idx;
			  ...
			  odp_crypto_ses_create_err_t *status)
{
	...
	if (session->p.cipher_iv.length > _ARM_CRYPTO_MAX_IV_LENGTH) {
		ODP_DBG("Maximum IV length exceeded\n");
		*status = ODP_CRYPTO_SES_CREATE_ERR_INV_CIPHER;
		goto err;
	}

	if (session->p.auth_iv.length > _ARM_CRYPTO_MAX_IV_LENGTH) {
